...
# Stores the scheduled wakeup call SID so we can cancel it
...
def incoming():
    ...
    gather.say(
        "Greetings! Thank you for calling Disney's All-Star Music Resort wakeup service. "
        "To continue in English, press star.",
        voice=VOICE
    )
    r.append(gather)
    r.say("We didn't receive your input. Goodbye!", voice=VOICE)
    return Response(str(r), mimetype="text/xml")

# Step 2: Language confirmed — show menu
@app.route("/language", methods=["GET", "POST"])
def language():
    digit = request.form.get("Digits", "")
    if digit != "*":
        return say("Invalid selection. Goodbye!")
    r = VoiceResponse()
    gather = Gather(num_digits=1, action="/menu", method="POST", timeout=10)
    gather.say(
        "Press one to set a new wakeup call. Press two to cancel your current wakeup.",
        voice=VOICE
    )
    r.append(gather)
    r.say("We didn't receive your input. Goodbye!", voice=VOICE)
...
def menu():
    ...
    if digit == "1":
        r = VoiceResponse()
        gather = Gather(num_digits=4, action="/get_time", method="POST", finish_on_key="#", timeout=15)
        gather.say(
            "Please enter the time for your wakeup, followed by the pound key. "
            "For example, for six A M, press zero six zero zero, then pound.",
            voice=VOICE
        )
        r.append(gather)
        r.say("We didn't receive your input. Goodbye!", voice=VOICE)
        ...
        if wakeup_job["sid"]:
            ...
            return say("Your wakeup call has been cancelled. Have a magical night!")
        else:
            return say("You have no wakeup call scheduled. Have a magical night!")
    else:
        return say("Invalid selection. Goodbye!")

# Step 4: Parse entered time and confirm
@app.route("/get_time", methods=["GET", "POST"])
def get_time():
    digits = request.form.get("Digits", "")
    if len(digits) != 4 or not digits.isdigit():
        return say("Invalid time entered. Please call back and try again. Goodbye!")
    ...
    gather.say(f"Press one for A M, press two for P M.", voice=VOICE)
    r.append(gather)
    r.say("We didn't receive your input. Goodbye!", voice=VOICE)
    return Response(str(r), mimetype="text/xml")

# Step 5: AM/PM selection and confirm
...
def get_ampm():
    ...
    gather.say(
        f"You entered {date_spoken}, {time_spoken}. "
        f"If this is correct, press one. To start over, press two.",
        voice=VOICE
    )
    r.append(gather)
    r.say("We didn't receive your input. Goodbye!", voice=VOICE)
...
def confirm():
    ...
    if not wake_time:
        return say("Something went wrong. Please call back. Goodbye!")
    ...
    full_date = now_str.strftime(f"%B {day_ord}, %Y")
    ...
    mickey_twiml = (
        ...
        f'<Say voice="{VOICE}">'
        f'The current time is {time_now} on the {day_ord} of {now_str.strftime("%B")}, {now_str.strftime("%Y")}.'
        f'</Say>'
        f'</Response>'
    )
    ...
    gather.say(
        f"You have a confirmed wakeup for {date_spoken}, {time_spoken}. "
        f"Press one to hear your current wakeup settings.",
        voice=VOICE
    )
    r.append(gather)
    r.say("Have a magical night!", voice=VOICE)
    return Response(str(r), mimetype="text/xml")
...
def after_confirm():
    ...
    if digit == "1" and wakeup_job["time_str"]:
        return say(
            f"Your current wakeup is set for {wakeup_job['date_str']} at {wakeup_job['time_str']}. "
            f"Have a magical night!"
        )
    return say("Have a magical night!")

# The actual wakeup call endpoint (called by Twilio at wakeup time)
@app.route("/ring")
def ring():
    now = datetime.now(ET)
    ...
    r.play(MICKEY_MP3)
    r.say(
        f"The current time is {time_now} on the {day_ord} of {now.strftime('%B')}, {now.strftime('%Y')}.",
        voice=VOICE
    )
    return Response(str(r), mimetype="text/xml")
